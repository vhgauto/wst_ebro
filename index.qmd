...
![Región de interés en el Delta del Ebro.](figuras/roi.png){#fig-roi}
...
En cada mapa de `WST` se mantuvo el mismo gradiente de color, a fin de facilitar la comparación entre fechas.
...
puntos_df <- data.frame(
  lon = c(0.663324, 0.748562, 0.870385),
  lat = c(40.649642, 40.776801, 40.734966),
  sitio = c("P1", "P2", "P3")
)
...
ggplot(d_wst, aes(fecha, wst, color = ID)) +
  geom_line() +
  geom_point() +
  scale_x_date(
    date_breaks = "3 month",
...
ggplot(d_wst, aes(ID, wst)) +
  geom_point(
    position = position_jitter(width = .2, seed = 2025), alpha = .4
  ) +
